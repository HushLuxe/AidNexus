...
export const parseAidIdFromQRCode = (data: string): string | null => {
  ...
  const deepLinkRegex = /^aidnexus:\/\/(?:testnet\/)?package\/([^\/?#]+)(?:[\/?#].*)?$/i;
  ...
  try {
    ...
    const hostMatches = /(^|\.)aidnexus\.org$/i.test(url.hostname);
...
export const ScannerScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleBarCodeScanned = ({ type, data }: { type: string; data: string }) => {
    ...
    Alert.alert(
      'Invalid QR Code',
      'This QR code is not a valid AidNexus package link. Please scan an AidNexus QR code.',
