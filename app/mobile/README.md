...
3. Build a development client or production build so the custom `aidnexus://` scheme is registered on the device.
...
- `aidnexus://` is configured as the mobile deep-link scheme so the wallet can return the user to AidNexus after approval.
