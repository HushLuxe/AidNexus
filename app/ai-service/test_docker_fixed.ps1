# Docker Test Script for AidNexus AI Service (PowerShell)
...
function Test-Build {
    ...
    try {
        $result = docker build -t aidnexus-ai-service:test --target production . 2>&1
        if ($LASTEXITCODE -ne 0) {
    ...
    try {
        ...
        $result = docker build -t aidnexus-ai-service:dev-test --target development . 2>&1
        if ($LASTEXITCODE -ne 0) {
...
function Test-Container {
    ...
    try {
        ...
        docker run -d --name ai-service-test `
            ...
            -e APP_ENV=test `
            aidnexus-ai-service:test | Out-Null
...
function Test-GPU {
    ...
    try {
        ...
        $result = docker build -t aidnexus-ai-service:gpu-test --target production-gpu . 2>&1
        if ($LASTEXITCODE -ne 0) {
...
function Invoke-Cleanup {
    ...
    # Remove test images
    docker rmi aidnexus-ai-service:test aidnexus-ai-service:dev-test aidnexus-ai-service:gpu-test 2>$null
...
function Invoke-Main {
    Write-Host "🐳 AidNexus AI Service Docker Test Script" -ForegroundColor Cyan
