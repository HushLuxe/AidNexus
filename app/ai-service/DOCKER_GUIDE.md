# Docker Deployment Guide for AidNexus AI Service

This guide provides comprehensive instructions for deploying the AidNexus AI Service using Docker with CUDA support.
...
### 1. Development Stage
```bash
docker build -t aidnexus-ai-service:dev --target development .
```
...
```bash
docker build -t aidnexus-ai-service:latest --target production .
```
...
```bash
docker build -t aidnexus-ai-service:gpu --target production-gpu .
```
...
1. **Build the image:**
```bash
docker build -t aidnexus-ai-service:latest .
```
...
- Container runs as non-root user `aidnexus`
...
### Image Updates
```bash
docker pull aidnexus-ai-service:latest
