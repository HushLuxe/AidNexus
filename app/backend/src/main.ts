...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('AidNexus API')
    .setDescription(
      `API documentation for AidNexus platform - Emergency aid and verification system
...
- Clients should monitor the API documentation for version updates`,
    ...
    .addServer('https://api.aidnexus.org/api/v1', 'Staging (v1)')
    .addServer('https://api.aidnexus.org/api/v1', 'Production (v1)')
    .build();

  const document = SwaggerModule.createDocument(app, config);
  SwaggerModule.setup('api/docs', app, document, {
    customSiteTitle: 'AidNexus API Docs',
    customfavIcon: 'https://aidnexus.org/favicon.ico',
